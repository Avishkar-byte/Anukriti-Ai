...
from ..requirements.models import RequirementPackage
from .schema import GraphEdge, GraphNode, NodeType, SystemGraph
...
Extract the system architecture graph for this device."""
            ...
            data = json.loads(raw)
            print(
                f"[GraphBuilder] ✅ LLM returned {len(data.get('components', []))} components, {len(data.get('edges', []))} edges"
            )
    ...
    def _parse_llm_graph(self, data: dict, package: RequirementPackage) -> SystemGraph:
        ...
        root_id = "SYS_ROOT"
        nodes.append(
            GraphNode(id=root_id, type=NodeType.COMPONENT, label=package.device_name)
        )
        ...
        for comp in data.get("components", []):
            ...
            edges.append(
                GraphEdge(source=root_id, target=comp["id"], relation="has_subsystem")
            )
        ...
        for comp in data.get("components", []):
            for req_id in comp.get("related_reqs", []):
                edges.append(
                    GraphEdge(
                        source=comp["id"], target=f"REQ_{req_id}", relation="satisfies"
                    )
                )

        return SystemGraph(
            nodes=nodes, edges=edges, metadata={"source": "llm", "model": self.model}
        )
    ...
    def _build_rule_based(self, package: RequirementPackage) -> SystemGraph:
        ...
        for req in package.requirements:
            ...
            edges.append(
                GraphEdge(source=root_id, target=req_node_id, relation="constrained_by")
            )
            ...
            if "battery" in req.description.lower():
                ...
                if not self.graph.has_node(comp_id):
                    self.graph.add_node(
                        comp_id, type=NodeType.COMPONENT, label="Battery Subsystem"
                    )
                    nodes.append(
                        GraphNode(
                            id=comp_id,
                            type=NodeType.COMPONENT,
                            label="Battery Subsystem",
                        )
                    )
                    edges.append(
                        GraphEdge(
                            source=root_id, target=comp_id, relation="has_subsystem"
                        )
                    )
                edges.append(
                    GraphEdge(source=comp_id, target=req_node_id, relation="satisfies")
                )

        return SystemGraph(nodes=nodes, edges=edges, metadata={"source": "rule_based"})
